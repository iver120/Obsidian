![[Pasted image 20260209163249.png]]


| Mengde  | 1      | 2          | 3    | 4    | 5    |
| ------- | ------ | ---------- | ---- | ---- | ---- |
| Pris    | 9.90*1 | 9.90*2     |      |      |      |
| forhold | 9.90/1 | (9.90*2)/2 | 9.90 | 9.90 | 9.90 |

![[Pasted image 20260209163554.png]]

Fordi pris delt på mengde alltid gir samme svar. Dette betyr også at vi har en lineær sammenheng mellom pris og mengde. 

![[Pasted image 20260209163836.png]]

9.90kr/hg er konstanten her. 

![[Pasted image 20260209163939.png]]


| d                  | 1    | 2      | 3      | 4     | 5      |
| ------------------ | ---- | ------ | ------ | ----- | ------ |
| O                  | 3.14 | 3.14*2 | 3.14*3 | 12.56 | 3.14*5 |
| O/d (forholdstall) | 3.14 | 3.14   | 3.14   | 3.14  | 3.14   |
![[Pasted image 20260209164213.png]]

$\pi \approx 3.14$
![[Pasted image 20260209164346.png]]

![[Pasted image 20260209164357.png]]

$$
P = 0.49x
$$
![[Pasted image 20260209164632.png]]
Her har vi to valg: Enten prøve å tolke en graf uten linjer, eller så kan vi finne noe konkret: 

siden hun tjener 1500 på 10, så må hun tjene halvparten på 5. 

![[Pasted image 20260209164947.png]]


y = 3x+10 --> lineær funksjon
y = ax + 0 --> proporsjonal størrelse 

| min     | 1         | 2   | 3    |
| ------- | --------- | --- | ---- |
| pris    | 1*3+10=13 | 16  | 19   |
| forhold | 13        | 8   | 6.33 |

# Funksjoner som proporsjonale størrelser 
## Oppgave 1 
Lag en funksjon som viser sammenhengen med antall timer en bil kjører i 60km/t og hvor langt bilen har kjørt totalt. 
f(x)=ax
$$
f(x)=60x
$$

Bruk denne funksjonen til å finne ut hvor langt bilen kjører på 3 timer og 30minutter. 
$$
f(3+30/60)=f(3.5)=60\cdot3.5 = 180+30=210
$$


Deretter til å finne ut hvor mange timer det tar før bilen har kjørt 2400km

$$
2400=60x
$$
$$
40=x
$$


## Oppgave 2
En klasse skal på tur til tyskland. De får ett tilbud på buss for 20000kr. Elevene må selv samle inn penger til dette. Hva må hver elev betale om det er 15 elever i klassen? 

$$
pris_{15} = \frac{20000}{15}
$$
Hvor mye må de betale om det er 20 elever? 
$$
pris_{20}=\frac{20000}{20}
$$

### omvendt proporsjonale størrelser som funksjoner
Kan du lage en funksjon som kan brukes i oppgaven over? P(x)

$$
P(x)=\frac{20000}{x}
$$
Bruk funksjonen til å finne prisen for 5 elever

$$
P(5)=\frac{20000}{5} = 4000
$$
Hvordan ville du skrevet en omvendt proporsjonal funksjon på en generell måte? 
$$
f(x) = \frac{a}{x}
$$
Hvor a er en konstant

Hva er definisjonsmengden for funksjonen i oppgaven over? 
$$
D_f=[15, 40]
$$

Vi forholder oss utelukkende til at x>0 her. 

## Oppgave 3
Kan du komme med ett eksempel fra hverdagen for både en proporsjonal størrelse og en omvendproporsjonal størrelse? 

Omvendtproporssjonal spleise på pizza
Proporsjonal Hvor langt en bil kjører på x-timer eller hvor mye det koster for x-av noe

Hva med noe som kan beskrives ved hjelp av en lineær funksjon? 
f(x)=ax+b

Alle typer situasjonen hvor man får en fast pris og deretter må betale per tid eller liknende med lønn

## Oppgave 3
Avgjør om den lineære funksjonen som går gjennom $A = (1, 1)$, $B=(2, 2)$

$$
y = ax+b
$$

$$
-x+y = 0
$$
$$
y = x
$$
Hvordan kunne vi funnet ut av dette på en sikker måte uten GeoGebra? 
hint: 
$$
a = \frac{y_2-y_1}{x_2-x_1}
$$






Oppgaver hentet fra NDLA: 
Aanensen, S., Kristensen, O., Holter, T. A. (2022, 30. april). Proporsjonalitet. NDLA. https://ndla.no/nb/r/matematikk-1p/proporsjonalitet/31c69b61ea